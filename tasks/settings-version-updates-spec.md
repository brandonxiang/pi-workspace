...
9. 升级遇到明确的权限错误时，使用固定命令 `sudo -n` 自动重试；不在网页表单中收集 sudo 密码。
10. `sudo -n` 需要交互认证时，在 Settings 中打开现有 PTY 终端并执行服务端生成的固定 sudo 命令，由 macOS 处理密码或 Touch ID；Settings 不读取或保存认证内容。
